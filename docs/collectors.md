# AWS Security Snapshot Tool - Collectors 문서화 (docs/collectors)
...
## 1. 공통 아키텍처 및 예외 처리 (Architecture & Design)

* **통합 실행 파이프라인**: 메인 스크립트인 `snapshot.py`가 각 수집기(collector) 모듈을 순차적으로 호출하여 전체 스냅샷을 구성합니다 [3, 5].
* **안전한 예외 처리 (`safe_call`)**: API 호출 시 권한 부족(`ClientError`)이나 리소스 부재 등의 에러가 발생하더라도 수집 프로세스 전체가 중단되지 않도록 예외를 안전하게 핸들링합니다 [4].
...
수집된 데이터는 `snapshot.py` 실행 시 지정한 인자(`before` 또는 `after`)에 따라 아래와 같은 디렉토리 구조로 JSON 저장됩니다 [4, 5]:
